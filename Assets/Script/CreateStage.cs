...
public class CreateStage : MonoBehaviour {
    ...
    public Transform default_stage;

    void Start () {
        /*
         * 初期座標の設定
         */
        pos = default_stage.position;
    ...
    public void Create(Vector3 pos, int ROOM_NUM) {
        ...
        foreach (char c in textdata)
        {
            ...
            {
                init_pos = new Vector3(pos.x, pos.y - 1, pos.z);
                floor_pos = new Vector3(pos.x, pos.y, pos.z);
        ...
        GameObject.Find("Awall").transform.localScale = new Vector3(0.75f, 0.75f, 0.75f);
        GameObject.Find("Astage").transform.localScale = new Vector3(0.75f, 0.75f, 0.75f);
        GameObject.Find("Apoint").transform.localScale = new Vector3(0.75f, 0.75f, 0.75f);

        

        objCount = 0;
        foreach (Transform obj2 in GameObject.Find("Apoint").transform)
        {
            objCount++;
            center += obj2.transform.position;
        }
        float maxX = 0f;
        float minX = 0f;
        foreach (Transform obj2 in GameObject.Find("Awall").transform)
        {

            if(obj2.position.x > maxX)
            {
                maxX = obj2.transform.position.x;
                if(minX == 0f)
                {
                    minX = maxX;
                }
            }
            if (obj2.position.x < minX)
            {
                minX = obj2.transform.position.x;
            }
        }

        maxX = Mathf.Floor(maxX)-1;
        minX = Mathf.Floor(minX)+1;
        foreach (Transform obj2 in GameObject.Find("Awall").transform)
        {
            if (obj2.position.x > maxX)
            {
        ...
        center = center / objCount;
        //center = GameObject.Find("Defencer").transform.position - center;

        GameObject.Find("Awall").transform.position -= center;
        GameObject.Find("Astage").transform.position -= center;
        GameObject.Find("Apoint").transform.position -= center;
        GameObject.Find("Awall").transform.position -= new Vector3(0, 0.05f,0);
        GameObject.Find("Astage").transform.position -= new Vector3(0, 0.05f, 0);
        GameObject.Find("Apoint").transform.position -= new Vector3(0, 0.05f, 0);
    }
